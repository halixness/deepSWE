...
del net
net = ResNet_VAE().to(device)
#net(th.Tensor(np.random.random((16, 3, 6, 128, 128))).to(device), summary=True).shape
...
for epoch in range(epochs):  # loop over the dataset multiple times
    ...
    for i, batch in enumerate(X_train):
        ...
        # ---- Loss and training
        loss = criterion(outputs,  masked_true_outputs)
        loss.backward()
        optimizer.step()
        ...
        if i == len(X_train)-1:
            # ---- Test loss
            z = np.random.randint(len(y_test), size=1)

            attention_mask_in = th.unsqueeze(X_test[z, :,1,:,:,:], 2)
            input = th.matmul(attention_mask_in, X_test[z])[0]

            outputs = net(input)

            attention_mask_out = th.unsqueeze(y_test[z,:,1,:,:,:],2)
            masked_true_outputs = th.matmul(attention_mask_out, y_test[z])

            test_loss = criterion(outputs,  masked_true_outputs)
            test_errors.append(test_loss.item())
            errors.append(loss.item())

            print('[%d, %5d] train_err: %.3f \t  test_err: %.3f \t avg_loss: %.3f' %
                  (epoch, i, errors[len(errors)-1], test_errors[len(test_errors)-1], np.mean(losses)))

    if epoch % 3 == 0:

        i = np.random.randint(len(X_test))

        attention_mask_in = th.unsqueeze(X_test[i,:,1,:,:,:], 1)
        input = th.matmul(attention_mask_in, X_test[i])
